...
本文档提供 Reachy Mini 机器人的核心调试方法、存储管理和网络下载策略。

---

## 目录

1. [核心调试方法论](#1-核心调试方法论手动接管模式-the-takeover-sop)
2. [存储危机应对](#2-存储危机应对16gb-sd-卡生存指南)
3. [下载与网络策略](#3-下载与网络策略矩阵)
4. [配置固化](#4-最终固化-persistence)

---

## 1. 核心调试方法论：手动接管模式 (The Takeover SOP)

当后台服务（Systemd）因网络或配置问题无法正常工作时，不要盲目重启。请按以下"三步走"接管控制权。
...
|-----|---------|------|------|-------------|
| **A. 自建/私有镜像** | **首选**。通过 `w0x7ce.eu` 下载 | 速度最快，无需代理 | 可能**缺包**（如 `red_light` 找不到文件） | `export PIP_INDEX_URL=https://pypi.w0x7ce.eu/simple`<br>`export HF_ENDPOINT=https://hf.w0x7ce.eu` |
| **B. 公共镜像** | **备选**。清华源/HF-Mirror | 速度快，包较全 | 偶尔会有同步延迟 | `export PIP_INDEX_URL=https://pypi.tuna.tsinghua.edu.cn/simple`<br>`export HF_ENDPOINT=https://hf-mirror.com` |
| **C. 官方源 + 代理** | **兜底**。当镜像缺包时使用 | **最稳**，绝对有包 | 速度受限于代理带宽，慢 | **1. 清除变量：** `unset HF_ENDPOINT`<br>**2. 加代理启动：** `proxychains4 python ...` |
